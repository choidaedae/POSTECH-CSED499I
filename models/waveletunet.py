...
class WaveDown(nn.Module):
    ...
    def forward(self, x, wave_x):
        x = self.maxpool(x)
        x = wave_x + x # concat or elementwise add 
...
class WaveletUNet(nn.Module):
    def __init__(self, n_channels, n_classes, wavelet_levels, bilinear=False):
...
if __name__ == "__init__":
    ...
    model = WaveletUNet(n_channels=4, n_classes=4, wavelet_levels=3, bilinear=True)
