...
class FeatureExtractorDDPM(FeatureExtractor):
    ...
    def _load_pretrained_model(self, model_path, **kwargs):
        ...
        self.device = kwargs['train']['run_device']
        self.model, self.diffusion = create_model_and_diffusion(**expected_args)
        self.model.to(dist_util.dev_num(self.device))
        if kwargs['use_fp16']:
            self.model.convert_to_fp16()
        self.model.eval()
        
    
    def collect_wavelet_features(self, args, freq_features):
        """ Upsample activations and concatenate them to form a feature tensor """
        
        wavelet_features = []
        for i in range(len(freq_features)):
            feature_list = []
            for level, freq_feature in enumerate(freq_features[i]):
                # feature wavelet, 120x120 resolution으로 만든 뒤 한 번에 / 혹은 resolution에 맞춰서 여러 번 
                feature = self.model.idwt_recurse(freq_feature, self.model.wavelets, level)
                feature_list.append(feature)
            features = torch.cat(feature_list, dim=1)
            wavelet_features.append(features)
        return torch.cat(wavelet_features, dim=1) #120, 60, 30, 15에서 IDWT한 feature들
    ...
    def forward(self, x, noise=None):
        ...
        for t in self.steps:
            ...
            wavelet, feature, _ = self.model(noisy_x, self.diffusion._scale_timesteps(t))
...
def _collect_features(args, activations: List[torch.Tensor]):
    """ Upsample activations and concatenate them to form a feature tensor """
    assert all([isinstance(acts, torch.Tensor) for acts in activations])
    size = tuple(args['dim'][:-1])
    resized_activations = []
