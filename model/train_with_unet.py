...
from src.pixel_classifier import load_ensemble, compute_iou, compute_dice, initialize_weights, FeatureVoter, UNet, UNetforFeature
...
def train(args):
    ...
    seg_net = UNet(args['dim'][-1], args['number_class']).to(dev_num(run_device))
    ...
    for epoch in range(train_args['epochs']): # epoch 
        ...
        for i, (image, label) in enumerate(train_loader): # iteration 
            ...
            #loss = l_match
            loss = l_seg + l_match + l_contrast # l_contrast: dot product
